...
import { Children, Result, TakenOrder, Tracer, UserBalance } from 'types';
...
export const SelectedTracerStore: React.FC<StoreProps> = ({ tracer, children }: StoreProps) => {
    ...
    const fetchUserData = async () => {
        if (tracers) {
            const userData = await Promise.all(Object.values(tracers).map((tracer: Tracer) => 
            ...
            if (matchingEngine === 1) {
                ...
                const makes: OrderData[] = [
                    {
                        ...
                        targetTracer:
                            selectedTracer?.address ? Web3.utils.toChecksumAddress(selectedTracer.address) : '',
                        nonce: 5101,
                    },
                ];
                ...
                if (orderType === 0) {
                    ...
                    if (!selectedTracer?.makeOrder) {
                        ...
                        handleTransaction
                            ? handleTransaction(selectedTracer.makeOrder, [rMargin, Math.round(price * 1000000), position === 0])
