...
export const Leverage: React.FC<LProps> = styled(({ leverage, orderDispatch, adjustLeverage, className, min, max }: LProps) => {
    return (
        <div className={`${className} m-3`}>
            ...
            <div className="w-3/4 pl-4 pr-6 pb-4 mt-2">
                <DefaultSlider
                    ...
                    handleChange={(num) => {
                        orderDispatch
                            ? orderDispatch({ type: (adjustLeverage ? 'setAdjustLeverage': 'setLeverage'), value: num })
