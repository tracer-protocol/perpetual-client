...
export type TooltipProps = {
    key: 'pool-holdings' | 'pool-target' | 'insurance-funding-rate' | 'realised-pnl' | 'unrealised-pnl' | 'amount' | 'price',
    props?: {
        baseTicker?: string,
...
type BTProps = { // BaseTickerProps
    baseTicker: string;
} & TProps;

export const AmountTip: React.FC<BTProps> = ({ baseTicker, className, children }: BTProps) => {
    const tooltip = (
        <p><strong>Amount</strong> Amount of {baseTicker} you would like to go long or short on for this order. If you have
        have an open position, your exposure will be updated by this specified amount.</p>
    )
    return (
...
export const PriceTip: React.FC<BTProps> = ({ baseTicker, className, children }: BTProps) => {
    const tooltip = (
        <p><strong>Price</strong> The price of {baseTicker} you would like to go long or short on. The order will execute at
...
export const BuyingPowerTip: React.FC<BPTProps> = ({ baseTicker, availableMargin, maxLeverage, className, children}: BPTProps) => {
    const tooltip = (
        <p>
            <strong>Buying Power</strong> Your maximum potential exposure to {baseTicker} given your available margin. It is
...
export const RealisedPnLTip: React.FC<BTProps> = ({ baseTicker, className, children }: BTProps) => {
    const tooltip = (
        <p>
            ...
            {baseTicker} position, and/or paying trading fees, funding rate payments, insurance funding rate payments and gas
            costs.
        </p>
    )
    return (
...
export const UnrealisedPnLTip: React.FC<BTProps> = ({ className, baseTicker, children }: BTProps) => {
    const tooltip = (
        <p>
            <strong>Unrealised PnL</strong> The amount of profit or loss you would realise if you closed your {baseTicker}{' '}
...
export const InsuranceFundingRateTip: React.FC<BTProps> = ({ baseTicker, className, children}: BTProps) => {
    const tooltip = (
        <p>
            ...
            {baseTicker} market to capitalise the {baseTicker} insurance pool. The fee is applied is continuously paid and updated
