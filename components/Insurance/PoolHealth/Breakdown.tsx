 ...
 *  
*/
const BreakdownBar: React.FC<BProps> = styled(({ 
    className
}: BProps ) => {
    return (
    ...
    > .liquidity {
        background: #3DA8F5;
        width: calc(${(props) => (props.liquidity-props.userBalance)/denom(props.target, props.liquidity) * 100}% - 4px);
    }
    > .userBalance {
        background: #005EA4;
        width: ${(props) => (props.userBalance)/denom(props.target, props.liquidity) * 100}%;
        border-top-right-radius: ${(props) => calcRemainder(props.target, props.liquidity, props.userBalance, props.buffer) === 0 ? '20px' : ''};
        border-bottom-right-radius: ${(props) => calcRemainder(props.target, props.liquidity, props.userBalance, props.buffer) === 0 ? '20px' : ''};
    }
    > .remainder {
