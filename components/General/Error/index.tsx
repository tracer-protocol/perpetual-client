...
export const MarginErrors: Record<string, ErrorBox> = {
    ...
    'WITHDRAW_INVALID': {
        ...
        moreInfo: 'You are attempting to withdraw funds that will cause your position to be liquidated. Decrease the amount you are trying to withdraw or close your position and then withdraw funds.'
    },
    'DEPOSIT_MORE': {
        name: 'Deposit Less than 150',
        message: 'You must deposit a minimum of $150 USD',
        moreInfo: 'The liquidation mechanism requires you to deposit a minimum of $150 USDC to ensure that you will be able to open a position without instantly being liquidated.'
    },
}
...
const Error: React.FC<EProps> = styled(({ className, error, message, context }: EProps) => {
    const error_ = error !== 'NO_ERROR' ? Errors[context ?? 'orders'][error] : {
        message: '',
        moreInfo: ''
    } ;
