...
const BASE_URL =
    process.env.NEXT_PUBLIC_OME_BASE_URL || 'http://localhost:8989';
 ...
 */
export const getOrders: (market: string) => Promise<Response> = async (
    market,
) => {
...
export const getUsersOrders: (
    market: string,
    account: string,
) => Promise<OMEOrder[]> = async (market: string, account: string) => {
...
const omefy: (str: string) => string = (str: string) =>
 ...
 */
export const createOrder: (market: string, data: OMEOrder) => Promise<Result> =
    async (market, data) => {
        if (!market) {
            return {
                status: 'error',
 ...
 */
export const getOrder: (market: string, orderId: string) => Promise<Response> =
    async (market, orderId) => {
 ...
 * @param orderId of the order being updated
 */
export const cancelOrder: (market: string, orderId: string) => Promise<Result> =
    async (market, orderId) => {
        return fetch(`${BASE_URL}/book/${omefy(market)}/order/${orderId}`, {
            method: 'DELETE',
        })
